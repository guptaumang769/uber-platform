package com.umang.uber.driver.config;
...
/**
 * Producer (driver-locations) + consumer (trip-events) wiring. A DefaultErrorHandler retries then
 * routes poison records to a DLT so one bad message can't wedge the consumer.
 */
@Configuration
@EnableKafka
public class KafkaConfig {
    ...
    public NewTopic driverLocationsTopic() {
        return TopicBuilder.name(KafkaTopics.DRIVER_LOCATIONS).partitions(3).replicas(1).build();
    }
    ...
    public ConcurrentKafkaListenerContainerFactory<String, String> stringListenerFactory(
        ...
        props.put(ConsumerConfig.GROUP_ID_CONFIG, "driver-service");
